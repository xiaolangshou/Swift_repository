...
// env object, like global application state
                ...
                ForEach(buttons, id:\.self) { row in
                    HStack(spacing: 12) {
                        ForEach(row, id: \.self) { button in
                            CalculatorButtonView.init(button: button)
                        }
                    }
                }
            }.padding(.bottom)
...
struct ContentView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ContentView().environmentObject(GlobalEnviroment.init())
    }
}
