...
extension Date {
    ...
    // 将时间戳转换为月日时分
    static func timeStampToStringDayHour(_ timeStamp:String) ->String {
    ...
    // 获取指定日期
    static func getDate(yearsToAdd: Int, monthsToAdd: Int, daysToAdd: Int) -> Date {
        
        var year = yearsToAdd
        var month = monthsToAdd
        var day = daysToAdd
        
        if day < 1 {
            month -= 1
            day = self.getCountOfDaysInMonth(year: year, month: month) + day
        } else if day > self.getCountOfDaysInMonth(year: year, month: month) {
            day -= self.getCountOfDaysInMonth(year: year, month: month)
            month += 1
        }
        ...
        // 指定日期零点时间戳
        let timeStamp = self.stringToTimeStamp("\(year)-\(month)-\(day) 00:00:00")
        
        // 将时间戳转换为年月日
        let dataStr = self.timeStampToString("\(timeStamp)")
        let calculatedDate = self.dateStringToDate(dataStr)
    ...
    // 比较两个时间戳之差，返回整型
    static func compareTwoTimeToInt(startTimeStamp: Int, nowTimeStamp: Int) -> Int {
    ...
    /// 小于3天: x天前发布
    /// 大于3天: 2015-12-02
    static func mapTimestampToAgo(_ timestamp: TimeInterval) -> String {
        let nowTimestamp = Date.init().timeIntervalSince1970
        let diffValue = nowTimestamp - timestamp
        if diffValue < Date.hour {
            return "\((diffValue / 60).int)分钟前发布"
        }
        if diffValue < Date.day {
            return "\((diffValue / hour).int)小时前发布"
        }
        if diffValue < Date.day * 3 {
            return "\((diffValue/day).int)天前发布"
        }
        
        dateFormatter.dateFormat = "yyyy-MM-dd"
        let str = dateFormatter.string(from: Date.init(timeIntervalSince1970: timestamp))
        return str
    }
}
